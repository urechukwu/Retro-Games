			...
			<div class="row">
				...
				<div class="col-md-8">
					...
					<div class="col-md-10">
						...
						<a href="img/products/5.jpg">
							 @if ($product->images->isNotEmpty())
							<img src="{{ asset('storage/' . $product->images->first()->image) }}" class="img-fluid" data-image>
							@else
...
@endif
				...
				<div class="col-md-4 mt-2">
					...
					<ul class="list-unstyled text-muted">
					  <li>Tags: <span>{{ $product->tags->pluck('name')->join(', '); }}</span></li>
					</ul>
					
					<div class="old-price">
						<span class="currency" data-currency>₦</span> <span data-price>{{$product->price + '500'}}</span>
					</div>
        ...
        <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
            <section class="container products clearfix" data-component-products="limit:4 page:0 id:1679,807,786,1597" data-products='{"1": "Mac pro", "2":"Ipod"}'>
                    ...
                    @foreach ($suggestedProducts as $suggestedProduct)
                        <div class="item" data-product>
                            <article class="product">
                                <a href="{{ route('view', $suggestedProduct->id) }}" data-url>
                                    @if ($suggestedProduct->images->isNotEmpty())
                                        <img src="{{ asset('storage/' . $suggestedProduct->images->first()->image) }}" class="img-fluid rounded-start" alt="Product Image">
                                    @else
                                        <p>No image available for this product.</p>
                                    @endif
                                </a>
                                <h3>
                                    <a href="{{ route('view', $suggestedProduct->id) }}" data-product-url data-name data-url>{{ $suggestedProduct->name }}</a>
                                </h3>
                                <div class="price-group">
                                    <div class="old-price">
                                        <span class="currency" data-product-currency>₦</span> 
                                        <span data-product-price>{{ $suggestedProduct->price + 500 }}</span>
                                    </div>
                                    <div class="price">
                                        <span class="currency" data-product-currency>₦</span> 
                                        <span data-product-price>{{ $suggestedProduct->price }}</span>
                                    </div>
                                </div>
                                <div class="btngroup">
                                    <a type="button" onclick="event.preventDefault(); document.getElementById('addtocart-{{ $suggestedProduct->id }}').submit();" class="btn btn-sm btn-secondary" title="Add to Cart" href="#" data-product-cart-url data-vvveb-action="addCart" data-product_id="{{ $suggestedProduct->id }}">
                                        <i class="la la-shopping-cart"></i> Buy Now
                                    </a>
                                    <form id="addtocart-{{ $suggestedProduct->id }}" action="{{ route('cart-add') }}" method="POST" style="display:none;">
                                        @csrf
                                        <input type="hidden" name="cart" value="{{ $suggestedProduct->id }}">
                                    </form>
                                </div>
                            </article><!-- product -->
                        </div> <!-- item -->
                ...
                <div class="d-grid gap-2 mt-3 text-center">
                    <a href="" class="btn btn-outline-secondary btn-md">See All</a>
                </div>
            </section> <!-- products -->
        </div>
    </div>
